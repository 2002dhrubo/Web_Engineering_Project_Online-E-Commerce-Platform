...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($name) || empty($brand) || $price <= 0) {
            ...
            } elseif (!empty($_POST['image_url']) && $_POST['image_url'] !== $product['image']) {
                // Use new external URL if provided
            ...
            if (empty($error)) {
                $stmt = $conn->prepare("UPDATE products SET name = ?, brand = ?, price = ?, image = ?, description = ?, category = ? WHERE id = ?");
                $stmt->bind_param("ssdsssi", $name, $brand, $price, $imagePath, $description, $category, $id);
                    ...
                    <form method="POST" enctype="multipart/form-data" class="admin-form">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="brand" name="brand" required 
                                       value="<?php echo escape($product['brand']); ?>"
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="description" name="description" rows="4" 
                                      placeholder="Enter product description"><?php echo escape($product['description']); ?></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label><i class="fas fa-image"></i> Current Image</label>
                            <?php if ($product['image']): ?>
                            <img src="<?php echo (strpos($product['image'], 'http') === 0) ? escape($product['image']) : '../' . escape($product['image']); ?>" 
                                 alt="Current image" class="current-image"
                                 onerror="this.src='https://via.placeholder.com/200x200?text=No+Image'">
                            <?php else: ?>
                            <p style="color: #888;">No image set</p>
                            <?php endif; ?>
                        </div>
                        
                        <div class="form-group">
                            <label for="image"><i class="fas fa-upload"></i> Upload New Image</label>
                            <input type="file" id="image" name="image" accept="image/jpeg,image/jpg,image/png">
                            <small style="color: #666; display: block; margin-top: 8px;">
                                Allowed: JPG, JPEG, PNG (Max: 5MB). Leave empty to keep current image.
                            </small>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" id="image_url" name="image_url" 
                                   value="<?php echo escape($product['image']); ?>"
